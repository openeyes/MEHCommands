...
CREATE DEFINER=`root`@`localhost` PROCEDURE shuffleContact()
  BEGIN
    ...
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    CREATE TEMPORARY TABLE IF NOT EXISTS temp_contact AS (SELECT * FROM contact);
    ...
    pc_loop: LOOP
      ...
      #SET @randomId = (SELECT (rand()*max(id)) FROM temp_contact);

      SET @randomId_fn = (SELECT id FROM contact WHERE (id % 1000) = floor(rand() * 1000) ORDER BY rand() LIMIT 1);
      SET @randomId_sn = (SELECT id FROM contact WHERE (id % 1000) = floor(rand() * 1000) ORDER BY rand() LIMIT 1);


      IF ( @randomId_fn IS NOT NULL) THEN
        SET @myTitle = (SELECT title FROM temp_contact WHERE id = @randomId_fn);
        SET @myFirstName = (SELECT first_name FROM temp_contact WHERE id = @randomId_fn);
      END IF;

      IF( (@randomId_sn IS NOT NULL ) AND (@randomId_fn != @randomId_sn) ) THEN
        SET @mySurname = (SELECT last_name FROM temp_contact WHERE id = @randomId_sn);
      END IF;

      UPDATE contact SET first_name=@myFirstName, title=@myTitle WHERE id=myId;
      UPDATE contact SET last_name=@mySurname WHERE id=myId;
    ...
    CLOSE pc_cursor;
    DROP TEMPORARY TABLE temp_contact;
