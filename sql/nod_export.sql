...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_surgeons()
  BEGIN
    SET @file = concat('/tmp/surgeons_', @time_now, '.csv');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_patients()
  BEGIN
    ...
    SET @file = concat('/tmp/patients_', @time_now, '.csv');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_patient_cvi_status()
  BEGIN
    ...
    SET @file = concat('/tmp/patient_cvi_status_', @time_now, '.csv');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_nod_episodes()
  BEGIN

    SET @file = concat('/tmp/episodes_', @time_now, '.csv');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episodes_diagnosis()
  BEGIN
    ...
    SET @file = concat('/tmp/episode_diagnosis_', @time_now, '.csv');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_diabetic_diagnosis()
  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_drug()
  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_biometry()
  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_iop()
  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_EpisodePreOpAssessment()
  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_refraction()
  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_visual_acuity()
  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_operation()
  BEGIN
    ...
    SET @file = concat('/tmp/episode_operation_', @time_now, '.csv');
    SET @cmd = ();
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_treatment()
  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE get_episode_treatment_cataract()
  BEGIN
    CREATE VIEW nod_episode_treatment_cataract AS SELECT pa.id,
                                                    (
                                                      ...
                                                      WHERE pl.event_id = pa.procedurelist_id
                                                      GROUP BY ep.patient_id
                                                      ORDER BY pl.last_modified_date desc

                                                    ) AS Eye,
                                                    NULL AS PreparationDrugId,
                                                    c.incision_site_id AS IncisionSiteId,
                                                    c.length AS IncisionLengthId,
                                                    4 AS IncisionPlanesId,
                                                    c.meridian AS IncisionMeridian
                                                  FROM ophtroperationnote_procedurelist_procedure_assignment pa
                                                    JOIN et_ophtroperationnote_procedurelist pl ON pa.procedurelist_id = pl.event_id
                                                    JOIN et_ophtroperationnote_cataract c ON c.event_id = pl.event_id;
  ...
  END
    ...
    call get_surgeons();
    #call get_patients();
    #call get_patient_cvi_status();
    #call get_nod_episodes();
   # call get_episodes_diagnosis();
   # call get_episode_diabetic_diagnosis();
   # call get_episode_drug();
   # call get_EpisodePreOpAssessment();
   # call get_episode_refraction();
    #call get_episode_visual_acuity();
   # call get_episode_treatment();
    #call get_episode_treatment_cataract();
