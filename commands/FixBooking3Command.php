...
class FixBooking3Command extends CConsoleCommand {
	public function run($args) {
		...
		foreach ($bookings as $booking) {
			if ($_booking = Yii::app()->db->createCommand()->select("*")->from("ophtroperationbooking_operation_booking")->where("id=:id",array(':id'=>$booking['id']))->queryRow()) {
				...
				if (strtotime($_booking['last_modified_user_id']) < strtotime("2013-05-29 21:50:00")) {
					$update['last_modified_user_id'] = $booking['last_modified_user_id'];
					$update['last_modified_date'] = $booking['last_modified_date'];
				}

				Yii::app()->db->createCommand()->update("ophtroperationbooking_operation_booking",$update,"id = {$_booking['id']}");
