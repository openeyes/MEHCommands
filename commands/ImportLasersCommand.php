		...
		do {
			while ($row = mssql_fetch_row($query)) {
				...
				if ( $site = Site::model()->find('short_name = ?', array($lkup_name)) ) {
					...
					if (!OphTrLaser_Site_Laser::model()->find('name = ? AND site_id = ?', array($row[0], $site->id) )) {
						$laser = new OphTrLaser_Site_Laser;
						$laser->name = $row[0];
						$laser->type = $row[1];
						...
						$criteria->limit = 1;
						if ($mx_row = OphTrLaser_Site_Laser::model()->find($criteria) ) {
